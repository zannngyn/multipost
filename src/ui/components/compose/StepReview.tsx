 ...
 *
 * One column, because the frame already owns the second one: the live Facebook
 * preview sits beside every step now, so the channel picker and the schedule
 * belong in the flow of this step rather than in a panel competing with it.
 * The action that fires them lives in the wizard footer.
 */
  ...
  return (
    <div className="flex min-w-0 flex-col gap-4">
      <section
        ...
        className="bg-card border-border flex flex-col gap-3.5 rounded-2xl border p-5"
      >
        ...
        {COMPOSE_CHANNELS.map((channel) => {
          ...
          return (
            <article key={channel.id} className="bg-muted rounded-xl p-4">
