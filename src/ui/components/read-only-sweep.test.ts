...
const GATED_SURFACES = [
  ...
  { screen: "Kênh", file: "channels/ConnectedChannelsScreen.tsx", writes: "bật/tắt, gỡ, nhập token" },
  { screen: "Đồng bộ", file: "sync/SyncScreen.tsx", writes: "chạy đồng bộ" },
] as const;

/** The two components that receive the reason instead of reading the hook. */
const PROP_FED_TABLES = [
  { component: "ScheduledJobTable", file: "scheduled/ScheduledJobTable.tsx" },
  { component: "JobLogTable", file: "jobs/JobLogTable.tsx" },
] as const;
...
describe("support mode: every write surface consults the read-only reason", () => {
  ...
  for (const table of PROP_FED_TABLES) {
    it(`${table.component} takes the reason from its screen`, () => {
      ...
      // These render the control, so they must receive the reason — a table
      // that ignores the prop would silently keep its buttons live.
      expect(source).toContain("readOnlyReason");
    });
  }
});
