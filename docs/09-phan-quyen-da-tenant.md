...
- [x] **M2.1** Tự tạo công ty: `POST /api/tenants` biên abuse 3+1/giờ trong advisory lock (test đua tất định), transaction đủ 4 bảng, cookie active sang công ty mới; UI `OnboardingPanel` + `TenantSwitcher` topbar — *commit d52bfa1, 20/08* ✅ *(M2.4 thay `OnboardingPanel` bằng `FirstRunGate` + `FirstRunWizard`; component cũ đã xoá)*
